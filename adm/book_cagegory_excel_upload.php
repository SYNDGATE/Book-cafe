...
if($_FILES['excelfile']['tmp_name']) {
	...
	if($DBzero) {
		sql_query("TRUNCATE g5_book_cagegory"); 
	}
    ...
    for ($i = 2; $i <= $data->sheets[0]['numRows']; $i++) {
				...
				$book_sortted= addslashes($data->sheets[0]['cells'][$i][2]);
		
		//도서목록 추가
			$sql = " insert into g5_book_cagegory
			            set 
							mb_id           = '$mb_id',
...
<div style="border:3px solid #000;padding:20px;max-width:700px;margin:50px auto 0 auto;">
	<h1 style="margin-bottom:40px;">엑셀데이터 데이터 INSERT!! < 번호 | 분류코드>  <a href="book\book_cagegory_table.xls" _blank>엑셀 샘플 파일</a> </h1>
		<form name="frm" id="frm" method="post" enctype="multipart/form-data">
			<input type="text" id="board_nm" name="board_nm" value="g5_book_cagegory" class="frm_input" style='width:120px;' placeholder="만들어질 테이블명">

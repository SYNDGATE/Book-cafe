...
add_stylesheet('<link rel="stylesheet" href="'.$board_skin_url.'/pop/pop.css">', 1);
...
<table width="<?php echo $width?>" bgcolor="#cfcfcf" border="0" cellspacing="1" cellpadding="5">
    ...
    while ($row = sql_fetch_array($result)) {  // 제목글 뽑아서 링크 문자열 만들기..
        ...
        for ($i = $start_day ; $i <= $end_day;  $i++) {
			...
			if($write_href)
				$links = G5_BBS_URL.'/board.php?bo_table='.$bo_table.'&year='.$year.'&month='.$month.'&wr_id='.$row['wr_id'].'&sc_no='.$sc_no;
			 else {
				 $links = "#";
				 if ($member['mb_level'] < $board['bo_read_level']) {
					$showLayer="" ;
				} else { 
					$showLayer='onclick="openPopup('.$row['wr_id'].')" ';
				}
			}
			$html_day[$i].= "<br /><img src=\"{$board_skin_url}/img/{$imgown}.gif\" align=\"absmiddle\"> <a href=\"{$links}\" id=\"subject_{$j}\" {$showLayer}>{$row['wr_subject']}</a>".$list['icon_new'].$list['comment_cnt'];
			?>
			<!-- 뷰 팝업레이어 -->
	...
	<div id="popup" class="popup">
	    <div class="thirteen"><h1><?php echo $g5['board_title']?> 미리보기</h1></div>
		<div class="frame" data-src="<?php echo $board_skin_url;?>/write_pop.php?bo_table=<?php echo $bo_table;?>&w=u&"></div>
	</div>
	<!-- 팝업리스트 End -->
...
<script src="<?php echo $board_skin_url;?>/pop/pop.js"></script>
